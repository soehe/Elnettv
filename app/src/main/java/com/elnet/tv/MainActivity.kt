...
class MainActivity : AppCompatActivity() {
    ...
    private val openPlaylist = registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri: Uri? ->
        ...
        try {
            ...
            showError("Gagal membaca file M3U")
        }
    }
    ...
    private fun loadFromUrl() {
        ...
        if (!url.startsWith("http://") && !url.startsWith("https://")) {
            showError("Masukkan URL playlist yang valid")
            return
        }
        statusView.visibility = View.VISIBLE
        statusView.text = "Memuat playlist..."
        Thread {
            try {
                ...
                runOnUiThread {
                    statusView.text = "Gagal memuat playlist; cache terakhir dikekalkan"
                    Toast.makeText(this, "Cek URL atau koneksi internet", Toast.LENGTH_LONG).show()
                }
            }
        }.start()
    }
    ...
    private fun loadPlaylist(text: String, sourceUrl: String?) {
        if (!text.trimStart().startsWith("#EXTM3U", ignoreCase = true)) {
            showError("Respons bukan playlist M3U yang valid")
            return
        }
        if (sourceUrl != null) {
            preferences.edit().putString("url", sourceUrl).putString("content", text).apply()
        }
        parsePlaylist(text)
    }
        ...
        for (i in lines.indices) {
            ...
            if (line.startsWith("#EXTINF", true)) {
                ...
                group = Regex("group-title=\\\"([^\\\"]*)\\\"", RegexOption.IGNORE_CASE)
                    .find(line)?.groupValues?.get(1)?.ifBlank { "Lainnya" } ?: "Lainnya"
                val url = lines.drop(i + 1).firstOrNull {
                    it.trim().isNotEmpty() && !it.trim().startsWith("#")
                }?.trim()
                if (!url.isNullOrBlank() && (url.startsWith("http://") || url.startsWith("https://"))) {
                    allChannels.add(Channel(title, url, group))
                }
            }
        }
        buildChannelMenu("")
    ...
    private fun buildChannelMenu(filter: String) {
        ...
        allChannels.filter {
            query.isBlank() || it.title.lowercase().contains(query) || it.group.lowercase().contains(query)
            ...
            val button = Button(this).apply {
                ...
                setOnFocusChangeListener { view, focused ->
                    view.setBackgroundColor(if (focused) Color.rgb(229, 9, 32) else Color.TRANSPARENT)
                }
            }
    ...
    private fun playChannel(index: Int) {
        ...
        statusView.visibility = View.VISIBLE
        statusView.text = "Memuat ${channel.title}..."
        ...
        player = ExoPlayer.Builder(this).build().also { exo ->
            ...
            exo.addListener(object : Player.Listener {
                ...
                override fun onPlayerError(error: PlaybackException) {
                    showError("Gagal memutar siaran")
                }
            })
        }
    }
